...
type SendTxArgs struct {
	...
	// Special transaction
	SpecialOp	bool
	SpecialArgs	blockchain.SpecialPayload
}
...
func handleWriteTx(w http.ResponseWriter, r *http.Request) {
	...
	if m.SpecialOp {
		if m.SpecialArgs.ResetAllAccountsState {
			tx = blockchain.BlockchainInstance.NewSpecialTransaction([]byte(m.Data), m.SpecialArgs)
		}
	} else {
		validate := wallet.Validate_Address(m.From, m.To, m.Value,blockchain.Wallets)
		if false == validate {
	...
	log.Printf("Total transactions count: %d", len(blockchain.BlockchainInstance.TxPool.AllTx))
...
func handleWriteBlock(w http.ResponseWriter, r *http.Request) {
	...
	newBlock := blockchain.GenerateBlock(blockchain.BlockchainInstance.Blocks[len(blockchain.BlockchainInstance.Blocks)-1], m.Msg, blockchain.BlockchainInstance.Proof, "")
